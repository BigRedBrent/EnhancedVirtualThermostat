 ...
 *
 *  Simplified Virtual Thermostat
 *
 *  Author: SmartThings, BigRedBrent
 */
definition(
    name: "Simplified Virtual Thermostat",
...
preferences {
	...
	section("How far away from the desired temperature before activating..."){
		input "threshold", "decimal", title: "Threshold", defaultValue: 0, required: true
	}
	section("Simulated temperature sensor(s) to copy the current temperature to... (not required)"){
		input "simulatedTemperatureSensor", "capability.temperatureMeasurement", title: "Simulated Temperature Sensor", multiple: true, required: false
	}
}
